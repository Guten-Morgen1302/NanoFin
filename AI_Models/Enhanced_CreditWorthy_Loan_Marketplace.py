...
# Try to get API key, use fallback if not available
try:
    GOOGLE_API_KEY = st.secrets["Gemini_API_Token"]
    if GOOGLE_API_KEY == "your_gemini_api_key_here":
        GOOGLE_API_KEY = None
        st.warning("⚠️ Gemini API key not configured. Using offline mode with limited features.")
    else:
        genai.configure(api_key=GOOGLE_API_KEY)
except Exception as e:
    GOOGLE_API_KEY = None
    st.info("ℹ️ Running in offline mode. Some AI features are disabled.")
...
def get_loan_types_from_gemini():
    ...
    # Return fallback data if API key is not available
        ...
        return {
            'additional_loans': [
                ...
                },
                {
                    ...
                    'required_documents': ['GST Returns', 'Bank Statements'],
                    'features': ['Fast Approval', 'Minimal Documentation'],
                    'upgrade_criteria': {
                        'min_credit_score': 65,
...
def main():
    st.title("💰 Enhanced NanoFin Loan Marketplace")
